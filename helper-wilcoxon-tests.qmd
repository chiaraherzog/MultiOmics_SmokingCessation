...
# df <- do.call(rbind, lapply(names(experiments(data)), function(a) {
#   data.frame(
#     assay = a,
#     x = rownames(experiments(data)[[a]]),
#     stringsAsFactors = FALSE
#   )
# }))
# v <- vars_list |> dplyr::anti_join(df)
...
df <- as.data.frame(longForm(df_filtered,
                             colData = c("age_at_consent", "interventionId",
                                         "subjectId", "visitId", "time",
                                         'compliance', 'smoking_py'))) |> 
...
We can view the output below using interactive tables, see [ST2](st2.qmd), [ST4](st4.qmd), [ST6](st6.qmd).