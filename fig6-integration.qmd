...
```{r sourcedata.covariates}
out <- p$data |> dplyr::mutate(id = match(subjectId, sample(unique(subjectId))),
                               age_at_consent = paste0(floor(age_at_consent / 5) * 5, "-", floor(age_at_consent / 5) * 5 + 4)) |>
  dplyr::arrange(id) |> dplyr::select(-c(compliance_smkgroup, mpstatrs, subjectId))
...
```{r sourcedata.covariates}
out <- p$data |> dplyr::mutate(id = match(subjectId, sample(unique(subjectId)))) |>
  dplyr::arrange(id) |> dplyr::select(-c(subjectId, sample, group))
...
out <- a$data |> dplyr::mutate(id = match(subjectId, sample(unique(subjectId)))) |>
  dplyr::arrange(id) |> dplyr::select(-c(subjectId))
...
out <- b$data |> dplyr::mutate(id = match(subjectId, sample(unique(subjectId)))) |>
  dplyr::arrange(id) |> dplyr::select(-c(subjectId))
...
out <- c$data |> dplyr::mutate(id = match(subjectId, sample(unique(subjectId)))) |>
  dplyr::arrange(id) |> dplyr::select(-c(subjectId))
...
out <- d$data |> dplyr::mutate(id = match(subjectId, sample(unique(subjectId)))) |>
  dplyr::arrange(id) |> dplyr::select(-c(subjectId))

out |> write.table(quote = F, sep = "\t", row.names = F, col.names = T,
                   file = 'srcdata/s9d.txt')
out <- e$data |> dplyr::mutate(id = match(subjectId, sample(unique(subjectId)))) |>
  dplyr::arrange(id) |> dplyr::select(-c(subjectId))
