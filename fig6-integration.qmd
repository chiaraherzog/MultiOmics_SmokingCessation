...
## Supplementary Figure 7 (MEFISTO)

![](figures-png/ext_figure_x_mefisto.png)

## Supplementary Figure 7 (rmcorr)
...
```{r sourcedata.bubbleplot}
out <- bubble_plot$data
...
corrtmp <- corr |> dplyr::mutate(x1 = paste0(assay_lab1, label1),
                                 x2 = paste0(assay_lab2, label2))
length(unique(c(corrtmp$x1, corrtmp$x2)))
...
length(unique(c(corrdiag$assay1, corrdiag$assay2)))
assays <- unique(c(corrdiag$assay1, corrdiag$assay2))
...
features = 'buccal_WID_SMK_proximal|buccal_WID_SMK_epithelial_hypoM|blood_WID_SMK_immune_hypoM|cervical_WID_SMK_epithelial_hypoM|cervical_WID_SMK_distal_epithelial_hypoM|cervical_WID_SMK_immune|PhenoAge|DamAge|AdaptAge|CausAge|AgeAccel|DunedinPACE|buccal_globalMethylation_line1|cervicalindex_CIN_adj'
...
## Supplementary Figure 8 (MEFISTO)
...
```{r sourcedata.variance}
out <- p$data

out |> write.table(quote = F, sep = "\t", row.names = F, col.names = T,
                   file = 'srcdata/s8a.txt')
```
...
```{r sourcedata.factor.plot}
out1 <- p1$data 
out2 <- p2$data
out <- out1 |> dplyr::left_join(out2) |> dplyr::select(-sample)

out |> write.table(quote = F, sep = "\t", row.names = F, col.names = T,
                   file = 'srcdata/s8b.txt')
```

* factor variance and significance association with selected covariates:

```{r}
metadat$visitId <- as.numeric(sub("M","", metadat$visitId)) # set visitID as ordered numeric
...
```{r sourcedata.covariates}
out <- p@matrix
out |> write.table(quote = F, sep = "\t", row.names = F, col.names = T,
                   file = 'srcdata/s8c.txt')
```
...
```{r sourcedata.covariates}
out <- p$data |> dplyr::group_by(subjectId) |> dplyr::mutate(id = dplyr::cur_group_id()) |> 
  dplyr::ungroup() |> dplyr::select(-c(subjectId, compliance_smkgroup, mpstatrs))
out |> write.table(quote = F, sep = "\t", row.names = F, col.names = T,
                   file = 'srcdata/s8d.txt')
```
...
```{r sourcedata.covariates}
out <- p$data |> dplyr::group_by(subjectId) |> dplyr::mutate(id = dplyr::cur_group_id()) |> 
  dplyr::ungroup() |> dplyr::select(-c(subjectId, sample))
out |> write.table(quote = F, sep = "\t", row.names = F, col.names = T,
                   file = 'srcdata/s8e.txt')
```
...
```{r, eval=F}
cairo_pdf("out/pdf/extxf-mefisto.pdf", width = 7, height = 5.5)
print(p)
dev.off()
```

```{r sourcedata.weights}
out <- p$data
out |> write.table(quote = F, sep = "\t", row.names = F, col.names = T,
                   file = 'srcdata/s8f.txt')
```


## Supplementary Figure 9 (rmcorr)
...
```{r sourcedata.crp.mono.rmcorr}
out <- a$data |> dplyr::group_by(subjectId) |> dplyr::mutate(id = dplyr::cur_group_id()) |> dplyr::ungroup() |> dplyr::select(-subjectId)

out |> write.table(quote = F, sep = "\t", row.names = F, col.names = T,
                   file = 'srcdata/s9a.txt')
```

### b) Cotinine WID-SMK hypoM

```{r}
ind = 37
m1 = paste0(corr$assay1[ind],"_", corr$measure1[ind])
m2 = paste0(corr$assay2[ind],"_", corr$measure2[ind])
lab1 = 'WID-SMK epithelial hypoM<br>(normalised)'
lab2 = 'Cotinine<br>(normalised)'
b <- rmcorr_plot(m1, m2, lab1, lab2)
```

```{r sourcedata.cotinine.hypom.rmcorr}
out <- b$data |> dplyr::group_by(subjectId) |> dplyr::mutate(id = dplyr::cur_group_id()) |> dplyr::ungroup() |> dplyr::select(-subjectId)

out |> write.table(quote = F, sep = "\t", row.names = F, col.names = T,
                   file = 'srcdata/s9b.txt')
```

### c) Butyricicoccaceae and proximal epithelial hyperM
...
lab2 = 'Butyricicoccaceae<br>(normalised)'
c <- rmcorr_plot(m1, m2, lab1, lab2)
c
```

```{r sourcedata.butyric.prox.rmcorr}
out <- c$data |> dplyr::group_by(subjectId) |> dplyr::mutate(id = dplyr::cur_group_id()) |> dplyr::ungroup() |> dplyr::select(-subjectId)

out |> write.table(quote = F, sep = "\t", row.names = F, col.names = T,
                   file = 'srcdata/s9c.txt')
```

### d-e) Adapt and DamAge

```{r}
ind = 137
m1 = paste0(corr$assay1[ind],"_", corr$measure1[ind])
m2 = paste0(corr$assay2[ind],"_", corr$measure2[ind])
lab1 = 'DamAge<br>(normalised)'
lab2 = 'CD94<sup>+</sup> cytotoxic T cells<br>(normalised)'
d <- rmcorr_plot(m1, m2, lab1, lab2)

ind = 579
m1 = paste0(corr$assay1[ind],"_", corr$measure1[ind])
m2 = paste0(corr$assay2[ind],"_", corr$measure2[ind])
lab1 = 'AdaptAge<br>(normalised)'
lab2 = 'CD94<sup>+</sup> cytotoxic T cells<br>(normalised)'
e <- rmcorr_plot(m1, m2, lab1, lab2)
```

```{r sourcedata.adapt.damage}
out <- d$data |> dplyr::group_by(subjectId) |> dplyr::mutate(id = dplyr::cur_group_id()) |> dplyr::ungroup() |> dplyr::select(-subjectId)

out |> write.table(quote = F, sep = "\t", row.names = F, col.names = T,
                   file = 'srcdata/s9d.txt')
out <- e$data |> dplyr::group_by(subjectId) |> dplyr::mutate(id = dplyr::cur_group_id()) |> dplyr::ungroup() |> dplyr::select(-subjectId)

out |> write.table(quote = F, sep = "\t", row.names = F, col.names = T,
                   file = 'srcdata/s9e.txt')
...
```{r}
plot <- (a|b|c)/(d|e|plot_spacer()) + plot_annotation(tag_levels = 'a')&
    theme(plot.tag = element_text(face = 'bold'))

cairo_pdf(here("figures-pdf/e9.pdf"), width = 10, height = 7)
print(plot)
dev.off()

ggsave(here("figures-png/e9.png"), plot = plot,
