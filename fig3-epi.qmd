...
h <- (buccal$plot + cervical$plot + blood$plot)
p <- draw(h, ht_gap = unit(2.5, "cm"))
```

```{r print.sharedheatmap, eval = F}
cairo_pdf(here("out/pdf/3c.pdf"), width = 9.5, height = 2.5)
print(p)
dev.off()
```
...
buccal_sep <- plot_lmm_heatmap_v3(lmm_data_time = time[match(ids, time$x),],
                              lmm_data_compliance = comp[match(ids, comp$x),],
                              relabel = labs,
                              relabel_assay = T,
                              filter_relabel = FALSE,
                              age_cor = F,
                              spy_cor = F,
                              cluster = F,
                              colour_assays = F)
...
print(buccal_sep$plot)
dev.off()
```

```{r sourcedata.buccal.heatmap}
buccal_sep$full_df |> write.table(quote = F, sep = "\t", row.names = F, col.names = T,
                   file = 'srcdata/3d.txt')
```
...
print(cervical_sep$plot)
dev.off()
```


```{r sourcedata.cervical.heatmap}
cervical_sep$full_df |> write.table(quote = F, sep = "\t", row.names = F, col.names = T,
                   file = 'srcdata/3e.txt')
```
...
```{r sourcedata.blood.heatmap}
blood_sep$full_df |> write.table(quote = F, sep = "\t", row.names = F, col.names = T,
                   file = 'srcdata/3f.txt')
```
