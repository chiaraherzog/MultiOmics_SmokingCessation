...
#     # right_join keeps every label, NA where tissue lacks that variable
#     dplyr::right_join(label_rows, by = "label") |>
#     dplyr::mutate(assay = tissue, assay2 = Outcome_cat) |>
#     dplyr::select(x, label, assay, assay2) |>
#     dplyr::arrange(assay2, label)      # identical ordering across tissues
# }
# 
# vars_keep_cervical <- make_keep("cervical")
# vars_keep_bu       <- make_keep("buccal")
# vars_keep_bl       <- make_keep("blood")
# 
...
#   lmm_data_time = vars_keep_cervical |> 
#     dplyr::left_join(time, by = c("x")) ,
#   lmm_data_compliance = vars_keep_cervical |> 
#     dplyr::left_join(comp, by = "x") ,
#   relabel = vars_keep_cervical,
#   relabel_assay = T,
...
#   lmm_data_time = vars_keep_bu |> 
#     dplyr::left_join(time, by = c("x")) ,
#   lmm_data_compliance = vars_keep_bu |> 
#     dplyr::left_join(comp, by = "x") ,
#   relabel = vars_keep_bu,
#   relabel_assay = T,
#   colour_assays = colour_assays
# )
# 
...
#   lmm_data_time = vars_keep_bl |> 
#     dplyr::left_join(time, by = c("x")) ,
#   lmm_data_compliance = vars_keep_bl |> 
#     dplyr::left_join(comp, by = "x") ,
#   relabel = vars_keep_bl,
#   relabel_assay = T,
#   colour_assays = colour_assays
# )
# 
...
time <- clean_lmm(out_lmm$`Basic model with packyears`)
comp <- clean_lmm(out_lmm$`Interaction model with packyears`)
...
p <- draw(plots$buccal$plot + plots$cervical$plot + plots$blood$plot)
```
...
### d/e) Smoking cessation: WID epithelial hypoM
...
cairo_pdf(here("out/pdf/3de.pdf"), width = 5.5, height = 4.25)
print(p)
dev.off()
```
...
### f/g buccal prox epithelial hyperM
...
out <- g$data |> dplyr::select(compliance, visitId, subjectId, value) |> dplyr::mutate(id = match(subjectId, sample(unique(subjectId)))) |>
  dplyr::arrange(id) |> dplyr::select(-subjectId)
...
### h/i) cervical distal immune hypoM
...
out <- l$data |> dplyr::select(compliance, visitId, subjectId, value) |> dplyr::mutate(id = match(subjectId, sample(unique(subjectId)))) |>
  dplyr::arrange(id) |> dplyr::select(-subjectId)
...
out <- i$data |> dplyr::select(compliance, visitId, subjectId, value) |> dplyr::mutate(id = match(subjectId, sample(unique(subjectId)))) |>
  dplyr::arrange(id) |> dplyr::select(-subjectId)
...
out <- smk.val$data |> dplyr::arrange(value) |> 
  dplyr::select(sampletype, type, rowname, value) |> 
  dplyr::arrange(sampletype, rowname)
