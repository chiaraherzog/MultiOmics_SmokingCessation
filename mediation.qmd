...
Smoking cessation -> saliva microbiome features -> saliva metabolites (significant wilcox)?

No significant mediators found.
...
Smoking cessation -> gut microbiome features -> urine metabolites (significant wilcox).
Test only genus/family level, ASV-level too personal.
...
ASVtable_stool <- readRDS("./out/ctrl_group_microbiome/1-output/ASVtable_stool_S.Rds") # S group only

# Note HIMA does ILR () transformation of the data, use fold changes (instead of simple delta) to avoid negative values

# Prepare ASV level count data
tax <- ASVtable_stool[,90:96]
tax$ASV <- ASVtable_stool$OTU
count_dat <- t(ASVtable_stool[,1:89] %>% tibble::column_to_rownames(var = "OTU"))
list_micro_ASV <- prep_data_hima(count_dat, pheno_stool)

# Prepare genus-level count data
d <- amp_load(otutable=ASVtable_stool, metadata=pheno_stool)
count_dat <- aggregate_abund(
    d$abund,
    d$tax,
    tax_aggregate = "Genus",
...
count_dat <- t(count_dat)
colnames(count_dat) <- gsub("\\[Eubacterium\\]","Eubacterium",colnames(count_dat))
list_micro_genus <- prep_data_hima(count_dat, pheno_stool)

# Prepare family-level count data
count_dat <- amp_load(otutable=ASVtable_stool, metadata=pheno_stool)
count_dat <- aggregate_abund(
    d$abund,
    d$tax,
    tax_aggregate = "Family",
    ...
    calcSums = FALSE)
count_dat <- t(count_dat)
list_micro_family <- prep_data_hima(count_dat, pheno_stool)
...
df <- wideFormat(data[,,41], colDataCols = c("interventionId", "subjectId", "visitId", "time", "compliance")) |>
  as.data.frame()
colnames(df) <- gsub("Urine.nuclear.magnetic.resonance..normalized_","",colnames(df))
...
# Log-fold changes metabolome data (HIMA will center mean to 0)
mtb <- df[,1:37] %>% tibble::column_to_rownames(var = "primary")
mtb_rel <- sweep(mtb, 1, rowSums(mtb), FUN = "/")
mtb_clr <- compositions::clr(mtb_rel + 1e-12) %>% as.data.frame()
...
eps <- 1e-12
delta_mtb_m2 <- mtb_m2[, -(37:38)] - mtb_m0[, -(37:38)] + eps # remove pheno columns
delta_mtb_m4 <- mtb_m4[, -(37:38)] - mtb_m0[, -(37:38)] + eps
delta_mtb_m6 <- mtb_m6[, -(37:38)] - mtb_m0[, -(37:38)] + eps
...
dir_out <- "~/Dropbox/eca/tirolgesund/7-manuscripts/smk1/smk-repository/out/mediation/gut_urine"
tax_level <- "genus"
pheno_dat <- pheno_dat
form_covar <- "~ X + age_at_consent + smoking_py"
mtb_interest <- c(
...
list_conf <- list(
  # smoking cessation at M2 -> gut microbiome at M2 -> metabolites at M2
  list(
    time_label = "M2M2M2",
    cessation_time = "M2",
    microbiome_dat = list_micro_genus[[1]],
  ...
  # smoking cessation at M2 -> gut microbiome at M2 -> metabolites at M4
  list(
    time_label = "M2M2M4",
    cessation_time = "M2",
    microbiome_dat = list_micro_genus[[1]],
  ...
  # smoking cessation at M2 -> gut microbiome at M4 -> metabolites at M4
  list(
    time_label = "M2M4M4",
    cessation_time = "M2",
    microbiome_dat = list_micro_genus[[2]],
  ...
  # smoking cessation at M2 -> gut microbiome at M4 -> metabolites at M6
  list(
    time_label = "M2M4M6",
    cessation_time = "M2",
    microbiome_dat = list_micro_genus[[2]],
  ...
  # smoking cessation at M2 -> gut microbiome at M6 -> metabolites at M6
  list(
    time_label = "M2M6M6",
    cessation_time = "M2",
    microbiome_dat = list_micro_genus[[3]],
  ...
  # smoking cessation at M4 -> gut microbiome at M4 -> metabolites at M4
  list(
    time_label = "M4M4M4",
    cessation_time = "M4",
    microbiome_dat = list_micro_genus[[2]],
  ...
  # smoking cessation at M4 -> gut microbiome at M4 -> metabolites at M6
  list(
    time_label = "M4M4M6",
    cessation_time = "M4",
    microbiome_dat = list_micro_genus[[2]],
  ...
  # smoking cessation at M4 -> gut microbiome at M6 -> metabolites at M6
  list(
    time_label = "M4M6M6",
    cessation_time = "M4",
    microbiome_dat = list_micro_genus[[3]],
  ...
  # smoking cessation at M6 -> gut microbiome at M6 -> metabolites at M6
  list(
    time_label = "M6M6M6",
    cessation_time = "M6",
    microbiome_dat = list_micro_genus[[3]],
