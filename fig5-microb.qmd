...
### Families - including control group
...
features <- c("interventionId", "subjectId", "visitId", "compliance", "compliance_smkgroup", "mpstatrs", "age_at_consent", "bmi_at_consent", "etohu_curr", "diet", "preg_ever", "ocp_curr", "hrt_curr")
...
```{r}
check <- ASVtable_saliva %>% filter(OTU %in% c("ASV_qwa_h7y_hxz","ASV_ivt_f41_sgz", "ASV_dvw_6oa_8gj")) %>% dplyr::select(!contains("SM")) %>%
...
```{r}
check <- ASVtable_stool %>% filter(OTU %in% c("ASV_hll_1bv","ASV_ea9_fnn", "ASV_pwc_dzt")) %>% dplyr::select(!contains("FM")) %>%
  dplyr::select(-Kingdom)
...
## family-level - including control group
...
# saliva
exp <- c("Saliva microbiome: families")
features <- c("mpstatrs", "age_at_consent", "bmi_at_consent", "etohu_curr", "diet", "preg_ever", "ocp_curr")
df_raw_wide <- as.data.frame(wideFormat(data[,,exp],
                                   colData = features)) %>%
  dplyr::mutate(sampleId = paste0(primary,"SM")) %>%
  dplyr::filter(sampleId %in% colnames(ASVtable_saliva_extend)[grepl("SM",colnames(ASVtable_saliva_extend))])
colnames(df_raw_wide) <- gsub("Saliva.microbiome..families_", "", colnames(df_raw_wide))

pheno <- pheno_saliva_extend %>%
  dplyr::select(sampleId,group)
features <- c("group", features)

df_raw_wide <- left_join(df_raw_wide,pheno) %>%
  dplyr::filter(group %in% c("Never smoker (control)","Smoker baseline"))
df_raw_wide <- df_raw_wide %>% dplyr::select(-sampleId)

out1 <- pc_microbiomes2(df_raw_wide,features,"hellinger",3,"Saliva",cols[2])
...
df_raw_wide <- as.data.frame(wideFormat(data[,,exp],
  ...
  dplyr::filter(sampleId %in% colnames(ASVtable_stool_extend)[grepl("FM",colnames(ASVtable_stool_extend))])
colnames(df_raw_wide) <- gsub("Stool.microbiome..families_", "", colnames(df_raw_wide))
...
df_raw_wide <- left_join(df_raw_wide,pheno) %>%
  dplyr::filter(group %in% c("Never smoker (control)","Smoker baseline"))
df_raw_wide <- df_raw_wide %>% dplyr::select(-sampleId)
out2 <- pc_microbiomes2(df_raw_wide,features,"hellinger",3,"Stool","#964B00")
...
### control vs M6 smokers vs M6 ex-smokers
...
df_raw_wide <- left_join(df_raw_wide,pheno) %>%
  dplyr::filter(group %in% c("Never smoker (control)","M6 no smoking cessation", "M6 smoking cessation"))
df_raw_wide <- df_raw_wide %>% dplyr::select(-sampleId)
...
tree <- "./out/ctrl_group_microbiome/3-output/saliva_S_ctrl_ASV_rootedcpr_treeML.nwk"
d <- amp_load(otutable=ASVtable_saliva_extend,
              metadata=pheno_saliva_extend)

table <- summarize_permanova_full(d,tree)

kable(table, format = "html", caption = "Saliva - PERMANOVA") %>%
...
# M0
plotsM0 <- plotPcoa(d0, "M0", "Aitchison", "group", "", NULL, "Saliva - Aitchison PCoA")
plotsM6 <- plotPcoa(d6, "M6", "Aitchison", "group", "", NULL, "")

p <- plotsM0[[1]] + plotsM6[[1]] + plot_layout(guides="collect")
p
...
cairo_pdf("out/pdf/5a.pdf", width = 6.5, height = 3)
print(p)
dev.off()
...
tree <- "./out/ctrl_group_microbiome/3-output/stool_S_ctrl_ASV_rootedarchaea_treeML.nwk"
d <- amp_load(otutable=ASVtable_stool_extend,
              metadata=pheno_stool_extend)

table <- summarize_permanova_full(d,tree)

kable(table, format = "html", caption = "Stool - PERMANOVA") %>%
  kable_styling(bootstrap_options = "striped", full_width = FALSE) %>%
  column_spec(1, bold = TRUE)  # Make the first column bold
...
# M0
plotsM0 <- plotPcoa(d0, "M0", "Aitchison", "group", "", NULL, "Stool - Aitchison PCoA")
plotsM6 <- plotPcoa(d6, "M6", "Aitchison", "group", "", NULL, "")
...
cairo_pdf("out/pdf/5b.pdf", width = 6.5, height = 3)
print(p)
dev.off()
...
table_print <- kable(table, format = "html", caption = "Extended Data Table 1. Overall and pair-wise permutational analysis of variance of the Aitchison distance for groups with different smoking status at baseline and months 2, 4 and 6 of the smoking cessation intervention.") %>%
  kable_styling(bootstrap_options = "striped", full_width = FALSE) %>%
  column_spec(1, bold = TRUE)  # Make the first column bold

kableExtra::save_kable(table_print, "figures-pdf/Extended_data_table2.pdf")
...
* no results after FDR correction for saliva
...
p <- plot_lmm_heatmap_frag_v3(exp = "Saliva microbiome: ASVs",
...
p <- plot_lmm_heatmap_frag_v3(exp = "Stool microbiome: ASVs",
                      lmm_data_time = out_lmm$`Interaction model`,
                      lmm_data_int = out_lmm$`Interaction model`,
                      relabel = dat_relab,
                      age_cor = T,
...
```
* This ASV is an outlier in the lower compliance group
* Check relative abundance for significant features (adjusted pval < 0.05 for any estimate):
...
* no results after FDR correction for saliva or stool
...
p <- plot_lmm_heatmap_time(exp = "Saliva microbiome: ASVs",
...
p <- plot_lmm_heatmap_time(exp = "Stool microbiome: ASVs",
                      lmm_data_time = out_lmm$`Higher compliance only`,
                      relabel = dat_relab,
                      age_cor = T,
                      spy_cor = T,
...
p <- plot_lmm_heatmap_frag_v3(exp = "Saliva microbiome: families",
                      lmm_data_time = out_lmm$`Interaction model`,
                      lmm_data_int = out_lmm$`Interaction model`,
                      relabel = dat_relab,
                      age_cor = T,
...
p <- plot_lmm_heatmap_frag_v3(exp = "Stool microbiome: families",
                      lmm_data_time = out_lmm$`Interaction model`,
                      lmm_data_int = out_lmm$`Interaction model`,
                      relabel = dat_relab,
                      age_cor = T,
...
* Check changes from baseline for significant features (pval < 0.01 for any plotted estimate):
...
# get time from basic model, combine with interaction from interaction model
dat1a <- as.data.frame(out_lmm$`Basic model with packyears`) %>%
    filter(str_detect(x, exp)) %>%
      dplyr::select(!contains(c("std.", "age_at_consent", "smoking_py"))) %>% 
      dplyr::select(!ends_with(c("compliance")))

dat1b <- as.data.frame(out_lmm$`Interaction model with packyears`) %>%
    filter(str_detect(x, exp))  %>%
      dplyr::select(!contains(c("std.", "age_at_consent", "smoking_py",
...
dat1 <- dplyr::full_join(dat1a,dat1b, by = "x")

rm(dat1a,dat1b);invisible(gc)
...
cairo_pdf("out/pdf/ext5c.pdf", width = 4.5, height = 1.5)
print(p)
dev.off()
...
cairo_pdf("out/pdf/ext5e.pdf", width = 6, height = 3.5)
print(p)
dev.off()
...
cairo_pdf("out/pdf/ext5d.pdf", width = 4.5, height = 1.5)
print(p)
dev.off()
...
cairo_pdf("out/pdf/ext5f.pdf", width = 4, height = 2)
print(p)
dev.off()
...
### Summary Interaction/Higher compliance

* Any significant feature pval < 0.01 in eigher Higher compliance model or interaction model (corrected packyears)
...
### gather significant families from interaction model
dat1 <- as.data.frame(out_lmm$`Interaction model with packyears`) %>%
    filter(str_detect(x, exp))
dat1$x = gsub(paste0(exp,"_"),"",dat1$x)
...
### gather significant families from high compliance model
dat1 <- as.data.frame(out_lmm$`Higher compliance only with packyears`) %>%
    filter(str_detect(x, exp))
dat1$x = gsub(paste0(exp,"_"),"",dat1$x)
...
toPlot <- unique(c(toPlot1,toPlot2))
rm(toPlot1,toPlot2); invisible(gc())

p <- boxplot_smokers_ctrl(dat1=NULL,
                          dat2=dat2,
                          exp=NULL,
                          pval=NULL,
                          interaction_only=NULL, 
                          taxa = toPlot)
...
* M0 correlation with smoking pack years

```{r}

# calculate ratio and add pheno
dat <- calc_FBratio(ASVtable_stool_extend) %>% 
  full_join(pheno_stool_extend, by = "sampleId") 
dat2 <- calc_CBratio(ASVtable_stool_extend)
dat <- full_join(dat,dat2, by = "sampleId") %>%
  filter(visitId == "M0")

p1 <- dat %>%
  ggplot(aes(x = smoking_py, y = FB_ratio)) +
  geom_point() +
  geom_smooth(method = "lm", se = FALSE, color = "blue") +
  stat_cor(method = "pearson", label.x = 0.5, label.y = 0.9) +
...
p2 <- dat %>%
  ggplot(aes(x = smoking_py, y = CB_ratio)) +
  geom_point() +
  geom_smooth(method = "lm", se = FALSE, color = "blue") +
  stat_cor(method = "pearson", label.x = 0.5, label.y = 0.9) +
  xlab("Smoking pack years") +
...
dat <- full_join(dat,dat2, by = "sampleId") %>%
  filter(compliance == "higher compliance") 

n <- dat %>% pull(subjectId) %>% unique() %>% length()

p1 <- plot_FBratio(dat = dat, 
             remove.outliers = F,
             subset = paste0("Stool, higher compliance (n=",n, ")"))
