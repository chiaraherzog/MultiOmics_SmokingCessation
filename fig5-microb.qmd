...
```{r sourcedata.asv.saliva}
out <- cbind(t(p$data), p$annot) |> 
...
```{r sourcedata.pcoa.coord}
out1 <- pb_M0$data |> dplyr::select(-pcoa3) |> dplyr::mutate(plot = 'control vs M0')
out2 <- pb_M6$data |> dplyr::select(-pcoa3) |> dplyr::mutate(plot = 'control vs M6')
out <- rbind(out1, out2)

out |> write.table(quote = F, sep = "\t", row.names = F, col.names = T,
                   file = 'srcdata/s6c.txt')
```
...
```{r sourcedata.pcoa.coord.stool}
out1 <- pb_M0$data |> dplyr::select(-pcoa3) |> dplyr::mutate(plot = 'control vs M0')
out2 <- pb_M6$data |> dplyr::select(-pcoa3) |> dplyr::mutate(plot = 'control vs M6')
out <- rbind(out1, out2)

out |> write.table(quote = F, sep = "\t", row.names = F, col.names = T,
                   file = 'srcdata/s6d.txt')
```
...
table_print <- kable(table, format = "html", caption = "Supplementary Table 8. Overall and pair-wise permutational analysis of variance of the Aitchison distance for groups with different smoking status at baseline and months 2, 4 and 6 of the smoking cessation intervention.") %>%
  kable_styling(bootstrap_options = "striped", full_width = FALSE) %>%
  column_spec(1, bold = TRUE)  # Make the first column bold
...
```{r sourcedata.salivaheatmap}
out <- p$full_df |> dplyr::select(-label)

out |> write.table(quote = F, sep = "\t", row.names = F, col.names = T,
                   file = 'srcdata/s6e.txt')
```
...
dat2 <- as.data.frame(longForm(data[,,exp], colData = c("interventionId", "subjectId", "visitId",'compliance')))
dat2 <- dat2 |>
...
cairo_pdf("out/pdf/ext6a.pdf", width = 6, height = 4)
print(p)
dev.off()

```

```{r sourcedata.abundance}
out <- p$data |> dplyr::select(-sampleId)

out |> write.table(quote = F, sep = "\t", row.names = F, col.names = T,
                   file = 'srcdata/s7a.txt')
```
...
```{r sourcedata.stoolheatmap}
out <- p$full_df |> dplyr::select(-label)

out |> write.table(quote = F, sep = "\t", row.names = F, col.names = T,
                   file = 'srcdata/s6f.txt')
```
...
cairo_pdf("out/pdf/ext6b.pdf", width = 4, height = 3.5)
print(p)
dev.off()

```

```{r sourcedata.abundance.stool}
out <- p$data |> dplyr::select(-sampleId)

out |> write.table(quote = F, sep = "\t", row.names = F, col.names = T,
                   file = 'srcdata/s7b.txt')
```
...
p1 <-  tmp %>%
  ggplot(aes(y = selbalScore_salivaSmok, x = group)) +
      ...
      theme_bw() +
      ylab('Microbiome smoking signature saliva') +
...
```{r sourcedata.selbal}
out1 <- p1$data |> dplyr::ungroup() |> dplyr::select(-sampleId)
out1 |> write.table(quote = F, sep = "\t", row.names = F, col.names = T,
                   file = 'srcdata/5c.boxplots.txt')

out2 <- p2$data |> 
  dplyr::mutate(id = match(subjectId, sample(unique(subjectId)))) |>
  dplyr::arrange(id) |> dplyr::select(-c(subjectId, primary))

out2 |> write.table(quote = F, sep = "\t", row.names = F, col.names = T,
                   file = 'srcdata/5c.longitudinal.txt')
```
...
p1 <-  tmp %>%
  ggplot(aes(y = selbalScore_stoolSmok, x = group)) +
      ...
      scale_fill_manual(values = cols_group, name = "", aesthetics = c('colour', 'fill')) +
      theme_bw() +
      ylab('Microbiome smoking signature stool') +
      scale_y_continuous(position = "right") +
      xlab('') +
      theme(legend.position = 'none') +
      coord_flip() 
...
```{r sourcedata.selbal.stool}
out1 <- p1$data |> dplyr::select(-sampleId)
out1 |> write.table(quote = F, sep = "\t", row.names = F, col.names = T,
                   file = 'srcdata/5d.boxplots.txt')
...
out2 |> write.table(quote = F, sep = "\t", row.names = F, col.names = T,
                   file = 'srcdata/5d.longitudinal.txt')
```
...
cairo_pdf("out/pdf/ext6c.pdf", width = 6, height = 5.5)
print(p)
dev.off()
...
cairo_pdf("out/pdf/ext6d.pdf", width = 4, height = 4)
print(p)
dev.off()
