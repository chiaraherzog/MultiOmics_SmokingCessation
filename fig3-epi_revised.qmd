...
```{r here}
here::i_am('fig3-epi_revised.qmd')
```
...
path <- "~/Dropbox/eca/tirolgesund/7-manuscripts/smk1/smk-repository/data/data_raw.Rdata"
...
path <- "~/Dropbox/eca/tirolgesund/7-manuscripts/smk1/smk-repository/data/data_baseline_change.Rdata"
...
path <- "~/Dropbox/eca/tirolgesund/7-manuscripts/smk1/smk-repository/data/data_normalized.Rdata"
...
path <- "~/Dropbox/eca/tirolgesund/7-manuscripts/smk1/smk-repository/data/data_normalized_baseline.Rdata"
...
pcHeatmap <- function(pc_dat){
  ...
  tmp$visitId <- as.numeric(sub("M","", tmp$visitId)) # set visitID as ordered numeric
...
```{r}
out <- ic_tissues$data |> 
  dplyr::select(sampletype, rowname, value, subjectId, visitId) |> 
  dplyr::group_by(subjectId) |> dplyr::mutate(id = dplyr::cur_group_id()) |> 
  dplyr::ungroup() |> 
...
colour_assays = list(
  assay = c(
    "Smoking"                          = "#C94C4C",
    "Cancer"                = "#7A5195",
    "Epigenetic clock"                = "#2F6B8A",
    "Functional epigenetic aging"     = "#E38B4A",
    "Global methylation"              = "#4FA3A5",
...
load("~/Dropbox/eca/tirolgesund/7-manuscripts/smk1/smk-repository/out/out_lmm_factor.Rdata")

#vars_keep <- readxl::read_xlsx(here("src/indices_info_completed.xlsx"))
# vars <- 
#   rbind(
#     readxl::read_xlsx(here("src/indices.xlsx"), sheet = 1),
#     readxl::read_xlsx(here("src/indices.xlsx"), sheet = 2)) %>%
#   rbind(readxl::read_xlsx(here("src/indices.xlsx"), sheet = 3)) %>%
#   distinct() %>%
#   dplyr::filter(label %in% vars_keep)
vars_keep <- readxl::read_xlsx("~/Dropbox/eca/tirolgesund/7-manuscripts/smk1/smk-repository/src/indices_info_completed.xlsx") %>%
  dplyr::select(label, Outcome_cat)
vars <- 
  rbind(
  ...
  distinct() %>%
  dplyr::filter(label %in% vars_keep$label) %>%
  dplyr::left_join(vars_keep, by = "label") %>%
  dplyr::mutate(x = gsub("_adj","",x)) %>%
  distinct()
...
vars_keep <- vars %>% 
  dplyr::mutate(x = paste0('cervical_', x),assay = 'cervical', assay2 = Outcome_cat) %>%
  dplyr::select(x, label, assay, assay2)

cervical <- plot_lmm_heatmap_v4(
  lmm_data_time = vars_keep %>%
    dplyr::left_join(time, by = c("x")) ,
  lmm_data_compliance = vars_keep %>%
    dplyr::left_join(comp, by = "x") ,
  relabel = vars_keep,
  relabel_assay = T,
...
vars_keep <- vars %>% 
  dplyr::mutate(x = paste0('buccal_', x),assay = 'buccal', assay2 = Outcome_cat) %>%
  dplyr::select(x, label, assay, assay2)

buccal <- plot_lmm_heatmap_v4(
  lmm_data_time = vars_keep %>%
    dplyr::left_join(time, by = c("x")) ,
  lmm_data_compliance = vars_keep %>%
    dplyr::left_join(comp, by = "x") ,
...
vars_keep <- vars %>% 
  dplyr::mutate(x = paste0('blood_', x),assay = 'blood', assay2 = Outcome_cat) %>%
  dplyr::select(x, label, assay, assay2)

blood <- plot_lmm_heatmap_v4(
  lmm_data_time = vars_keep %>%
    dplyr::left_join(time, by = c("x")) ,
  lmm_data_compliance = vars_keep %>%
    dplyr::left_join(comp, by = "x") ,
...
buccal$plot@left_annotation <- NULL
blood$plot@left_annotation <- NULL

p <- cervical$plot + buccal$plot + blood$plot

draw(p)

# Heading titles above doesnt work, will have to do this manually

```

```{r print.sharedheatmap, eval = F}
#cairo_pdf(here("out/pdf/3c_revised.pdf"), width = 9.5, height = 4)
cairo_pdf("~/Dropbox/eca/tirolgesund/7-manuscripts/smk1/smk-repository/out/pdf/3c_revised.pdf", width = 9.5, height = 5)
print(p)
dev.off()
```

```{r sourcedata.shared.heatmap}
buccal$full_df |> write.table(quote = F, sep = "\t", row.names = F, col.names = T,
                   file = 'srcdata/3c.buccal.txt')

cervical$full_df |> write.table(quote = F, sep = "\t", row.names = F, col.names = T,
                   file = 'srcdata/3c.cervical.txt')

blood$full_df |> write.table(quote = F, sep = "\t", row.names = F, col.names = T,
...
path = here("data/data_raw.Rdata")
df <- as.data.frame(longForm(get(load(path))[,,c("Composite methylation scores: buccal",
...
```{r sourcedata.smk}
out <- d$data |> dplyr::select(type, value)
out |> write.table(quote = F, sep = "\t", row.names = F, col.names = T,
                   file = 'srcdata/3g.txt')

out <- e$data |> dplyr::select(compliance, visitId, subjectId, value) |> 
...
```{r sourcedata.smk.prox}
out <- f$data |> dplyr::select(type, value)
out |> write.table(quote = F, sep = "\t", row.names = F, col.names = T,
                   file = 'srcdata/3i.txt')

out <- g$data |> dplyr::select(compliance, visitId, subjectId, value) |> 
  dplyr::group_by(subjectId) |> dplyr::mutate(id = dplyr::cur_group_id()) |> dplyr::ungroup() |> dplyr::select(-subjectId)
...
### k/l) cervical distal immune hypoM

```{r fig.width = 8, fig.height = 6.5, eval = F}
index = indices[11]
ylab = '<b>WID-SMK, distal epithelial hyperM</b><br><b style="color:#bd647d">cervical</b><br>(scaled and corrected)'

dat <- df |> 
  dplyr::filter(rowname %in% index)
lims = c(min(dat$value)*1.01, max(dat$value*1.01))
...
k
l <- paired_longitudinal_compliance(df_change_blood,
                               variable = gsub("cervical_", "", index),
                               p = 'p.signif',
                               ylab = paste0("<b>∆</b> ", ylab)) +
  theme(aspect.ratio = NULL)
```

```{r plot.smk.cerv, eval = F}
p <- (k | l) + plot_layout(widths = c(0.9, 1.3))

cairo_pdf(here("out/pdf/3kl.pdf"), width = 5.5, height = 4.25)
print(p)
dev.off()
```

```{r sourcedata.smk.cerv}
out <- k$data |> dplyr::select(type, value)
out |> write.table(quote = F, sep = "\t", row.names = F, col.names = T,
                   file = 'srcdata/3k.txt')

out <- l$data |> dplyr::select(compliance, visitId, subjectId, value) |> 
  dplyr::group_by(subjectId) |> dplyr::mutate(id = dplyr::cur_group_id()) |> dplyr::ungroup() |> dplyr::select(-subjectId)
...
```{r sourcedata.smk.imm}
out <- h$data |> dplyr::select(type, value)
out |> write.table(quote = F, sep = "\t", row.names = F, col.names = T,
                   file = 'srcdata/3m.txt')

out <- i$data |> dplyr::select(compliance, visitId, subjectId, value) |> 
  dplyr::group_by(subjectId) |> dplyr::mutate(id = dplyr::cur_group_id()) |> dplyr::ungroup() |> dplyr::select(-subjectId)
...
### m) AgeAccelGrimV2 (not shown after revision)
...
```{r sourcedata.grim}
out <- grim$data |> dplyr::select(visitId, subjectId, value) |> 
  dplyr::group_by(subjectId) |> dplyr::mutate(id = dplyr::cur_group_id()) |> dplyr::ungroup() |> dplyr::select(-subjectId)
...
```{r sourcedata.scree}
scree |> write.table(quote = F, sep = "\t", row.names = F, col.names = T,
                   file = 'srcdata/s4a.txt')
```
...
```{r sourcedata.pc.ic}
pc_ic |> write.table(quote = F, sep = "\t", row.names = F, col.names = T,
                   file = 'srcdata/s4b.txt')
```
...
```{r sourcedata.smk.sigs}
out <- smk.val$data |> 
  dplyr::select(sampletype, type, rowname, value)

out  |> write.table(quote = F, sep = "\t", row.names = F, col.names = T,
                   file = 'srcdata/s4c.txt')
```
...
```{r sourcedata.heatmap}
m |> write.table(quote = F, sep = "\t", row.names = F, col.names = T,
                   file = 'srcdata/s4d.txt')
```
...
time <- out_lmm$`Basic model with packyears`  |> 
  dplyr::filter(grepl("Composite", x) & grepl("DNAm", x) & !grepl("DNAmTL|AgeAccelGrimV2", x) & grepl("blood", x)) |> #? these are not there?
  dplyr::filter(grepl("Composite", x) & grepl("blood", x)) |> 
  dplyr::mutate(x = gsub("Composite methylation scores: ", "", x)) 
                
comp <- out_lmm$`Interaction model with packyears` |> 
  dplyr::filter(grepl("Composite", x) & grepl("DNAm", x) & !grepl("DNAmTL|AgeAccelGrimV2", x) & grepl("blood", x))|> 
  dplyr::mutate(x = gsub("Composite methylation scores: ", "", x))
...
```{r sourcedata.grim}
heatmap$full_df |> write.table(quote = F, sep = "\t", row.names = F, col.names = T,
                   file = 'srcdata/s4e.txt')
```
...
```{r sourcedata.relative}
out <- a$data|> dplyr::select(rowname, subjectId, value, nic_replacement, t_relative) |> 
  dplyr::group_by(subjectId) |> dplyr::mutate(id = dplyr::cur_group_id()) |> dplyr::ungroup() |> dplyr::select(-subjectId)
out |> write.table(quote = F, sep = "\t", row.names = F, col.names = T,
                   file = 'srcdata/s4f.blood.imm.hypoM.txt')

out <- b$data|> dplyr::select(rowname, subjectId, value, nic_replacement, t_relative) |> 
  dplyr::group_by(subjectId) |> dplyr::mutate(id = dplyr::cur_group_id()) |> dplyr::ungroup() |> dplyr::select(-subjectId)
out |> write.table(quote = F, sep = "\t", row.names = F, col.names = T,
                   file = 'srcdata/s4f.buccal.epi.hypoM.txt')

out <- c$data|> dplyr::select(rowname, subjectId, value, nic_replacement, t_relative) |> 
  dplyr::group_by(subjectId) |> dplyr::mutate(id = dplyr::cur_group_id()) |> dplyr::ungroup() |> dplyr::select(-subjectId)
out |> write.table(quote = F, sep = "\t", row.names = F, col.names = T,
                   file = 'srcdata/s4f.buccal.prox.epi.hyperM.txt')
