trainImmAge_smoking_info <- function(data, exp, populations,
                            mainOnly = F){
  
  # Extract data ------
  tr <- as.data.frame(wideFormat(data[,data@colData$visitId == 'M0',
                                      exp],
                                 colData = c('age_at_consent', 'interventionId'))) |> tidyr::drop_na()
  colnames(tr) <- gsub("Flow.cytometry..T.cell.staining_|Flow.cytometry..white.blood.cell.staining_", "", colnames(tr))
  
  trdata <- tr |> 
    dplyr::mutate(smk = ifelse(interventionId == 'S', 1, 0)) |> 
    dplyr::select(-c('age_at_consent', 'primary'))
  
  if(mainOnly == T){
    trdata <- trdata |> 
      dplyr::select(any_of(c(dplyr::filter(populations, main_analysis == 'yes')$name, 'smk')))
  }
