---
title: "Clinical and immune changes (F2, S2, S3)"
format:
  html:
    toc: true
    code-fold: true
...
```{r sourcedata.scree, eval = F}
out <- tmp |> 
  dplyr::arrange(dplyr::desc(`percentage of variance`)) |> 
  dplyr::slice(1:10) |> 
...
```{r sourcedata.pcbi, eval = F}
out.coord <- pc.clin$data |> 
  dplyr::select(Groups, name, x, y)

out.contrib <- res.pca$var$contrib |> as.data.frame() |> dplyr::arrange(dplyr::desc(Dim.1)) |> dplyr::slice(1:5)
out.contrib2 <- res.pca$var$contrib |> as.data.frame() |> dplyr::arrange(dplyr::desc(Dim.2)) |> dplyr::slice(1:5)

out.coord |> write.table(quote = F, sep = "\t", row.names = F, col.names = T,
                   file = 'srcdata/2b.coord.txt')

out.contrib <- rbind(out.contrib, out.contrib2)

out.contrib |> write.table(quote = F, sep = "\t", row.names = F, col.names = T,
                   file = 'srcdata/2b.contrib.txt')
```
...
```{r sourcedata.pcbi, eval = F}
out <- rbind(pc1$data, pc2$data, pc3$data, pc4$data)
...
```{r sourcedata.2e}
out <- as.data.frame(pc$biplot$data) |> 
  dplyr::select(name, `Col.`, x, y) |> 
  dplyr::mutate(subjectId = substr(name, 1, 4)) |> 
  dplyr::mutate(id = match(subjectId, sample(unique(subjectId)))) |> 
  dplyr::arrange(id) |> dplyr::select(-c(name, subjectId))

out.contrib <- pc$pc.object$var$contrib |> as.data.frame() |> dplyr::arrange(dplyr::desc(Dim.1)) |> dplyr::slice(1:5)
out.contrib2 <- pc$pc.object$var$contrib |> as.data.frame() |> dplyr::arrange(dplyr::desc(Dim.2)) |> dplyr::slice(1:5)

out |> write.table(quote = F, sep = "\t", row.names = F, col.names = T,
                   file = 'srcdata/2e.coord.txt')
out.contrib <- rbind(out.contrib, out.contrib2)
out.contrib |> write.table(quote = F, sep = "\t", row.names = F, col.names = T,
                   file = 'srcdata/2e.contrib.txt')
```
...
```{r sourcedata.immheatmap}
out <- pc$heatmap@matrix
...
df <- longForm(data[,,44], colDataCols = c("interventionId", "subjectId", "visitId", "time", "compliance")) |> 
  as.data.frame() |> 
...
a <- paired_longitudinal_compliance(df,
                               variable = 'ImmAge_gen',
                               p = 'p.signif',
                               ylab = ylab,
                               nn = 4) &
...
```{r}
out <- a$data |> dplyr::select(subjectId, visitId, value, compliance) |>   
  dplyr::mutate(id = match(subjectId, sample(unique(subjectId)))) |>
...
df <- longForm(data[,,45], colDataCols = c("interventionId", "subjectId", "visitId", "time", "compliance")) |> 
  as.data.frame() |> 
  dplyr::filter(interventionId == 'S' & ! visitId %in% c("M12", "M18")) |> 
  dplyr::mutate(compliance = factor(compliance, levels = c("lower compliance", "higher compliance"))) |> 
  dplyr::filter(!is.na(value))
...
```{r}
out <- immsmk_change$data |> dplyr::select(subjectId, visitId, value, compliance) |>  
  dplyr::mutate(id = match(subjectId, sample(unique(subjectId)))) |>
  dplyr::arrange(id) |> dplyr::select(-subjectId)
...
```{r sourcedata.hist}
out <- corr |> dplyr::select(pop_name, cor, p)

out |> write.table(quote = F, sep = "\t", row.names = F, col.names = T,
                   file = 'srcdata/s2a.txt')
```
...
out <- corr |> dplyr::select(pop_name, cor, p) |> 
  dplyr::mutate(padj = p.adjust(p, method = 'holm'),
                sig = ifelse(p<0.05, pop_name, NA)) |> dplyr::select(-padj)

out |> write.table(quote = F, sep = "\t", row.names = F, col.names = T,
                   file = 'srcdata/s2b.txt')
```
...
```{r sourcedata.heat.wb}
out <- wb_heat$full_df

out |> write.table(quote = F, sep = "\t", row.names = F, col.names = T,
                   file = 'srcdata/s2c.txt')
```
...
```{r sourcedata.coef}
out <-tmp |> dplyr::select(pop_name, s1)

out |> write.table(quote = F, sep = "\t", row.names = F, col.names = T,
                   file = 'srcdata/s3a.txt')
```
...
df <- longForm(data[,,45], colDataCols = c("interventionId", "subjectId", "visitId", "time", "compliance", "cig_curr", "smoking_py")) |> 
  as.data.frame() |> 
...
```{r sourcedata}
out <- s1b$data |> dplyr::select(value, smoking_current)
out |> write.table(quote = F, sep = "\t", row.names = F, col.names = T,
                   file = 'srcdata/s3b.txt')
```
...
```{r sourcedata.roc}
coords  |> write.table(quote = F, sep = "\t", row.names = F, col.names = T,
                   file = 'srcdata/s3c.txt')
```
...
```{r sourcedata.cor}
out <- df |> dplyr::select(cig_curr, value)
out |> write.table(quote = F, sep = "\t", row.names = F, col.names = T,
                   file = 'srcdata/s3d.txt')
```
...
```{r}
out <- df |> dplyr::select(smoking_py, value)
out |> write.table(quote = F, sep = "\t", row.names = F, col.names = T,
                   file = 'srcdata/s3e.txt')
```

### f) ImmuneAge at baseline


```{r}
load(here("data/data_raw.Rdata"))

df <- longForm(data[,,44], colDataCols = c("interventionId", "subjectId", "visitId", "time", "compliance", "cig_curr", "smoking_py")) |> 
  as.data.frame() |> 
  dplyr::filter(visitId == 'M0') |> 
  dplyr::mutate(compliance = factor(compliance, levels = c("lower compliance", "higher compliance"))) |> 
  dplyr::filter(!is.na(value)) |> 
...
s1f <- df |> 
  dplyr::filter(rowname == 'ImmAge_gen_adj') |> 
  ggplot(aes(x = smoking_current,
             y = value)) +
  gghalves::geom_half_boxplot(aes(fill = smoking_current,
                                  colour = smoking_current),
  ...
  scale_fill_manual(values = cols[c(8, 6)],
                    ...
                    aesthetics = c('colour', 'fill')) +
  labs(x = 'Current smoking',
       y = '<b>ImmuneAge</b><br>(arbitrary units;<br>adjusted for chronological age)') +
  guides(fill = guide_legend(nrow = 3),
         colour = guide_legend(nrow = 3)) +
  theme_bw() +
  theme(axis.title.y = element_markdown(),
...
out <- df |> 
  dplyr::filter(rowname == 'ImmAge_gen_adj') |> 
  dplyr::select(smoking_py, value) 
out |> write.table(quote = F, sep = "\t", row.names = F, col.names = T,
                   file = 'srcdata/s3f.txt')
```
...
plot <- (s1a+s1b+s1c+s1d+s1e+s1f) + plot_layout(design = design) +
